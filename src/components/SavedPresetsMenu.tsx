...
export function SavedPresetsMenu({
  ...
  useEffect(() => {
    const mediaQuery = window.matchMedia("(max-width: 519px)");
  ...
  const popoverLayer = open ? (
    <div ref={popoverRef} className="presets-popover-layer">
      <button
        type="button"
  ...
  return (
    <div className={`presets-menu ${open ? "presets-menu--open" : ""}`} ref={menuRef}>
      ...
      {popoverLayer &&
        (isMobile ? createPortal(popoverLayer, document.body) : popoverLayer)}
